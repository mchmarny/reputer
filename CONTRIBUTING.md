...
2. Check existing [issues](https://github.com/mchmarny/reputer/issues) to avoid duplicates
...
- Use [GitHub Issues](https://github.com/mchmarny/reputer/issues/new) to report bugs
...
- Open a [GitHub Issue](https://github.com/mchmarny/reputer/issues/new) describing the feature
...
These principles guide all design decisions in reputer. When faced with trade-offs, these principles take precedence.
...
**What:** GitHub/GitLab API calls may fail, rate limits may be hit, network may be unreliable. Every external interaction must handle failure gracefully.

**Why:** Git provider APIs are the primary external dependency. Resilience is not optional.

### Boring First

Default to proven, simple technologies.

**What:** Standard library where possible, `log/slog` for logging, `testify` for assertions. No frameworks, no unnecessary abstractions.
...
- Use `log/slog` with structured key-value pairs for logging
...
- **GitHub Issues**: [Create an issue](https://github.com/mchmarny/reputer/issues/new) with the "question" label
...
Thank you for contributing to reputer!
